  ...
  return (
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
      ...
      <div className="w-[40px] h-[40px] rounded-[10px] bg-[#f3f3f8] flex justify-center items-center cursor-pointer">
            <img src={dash_logo} alt="user" className="w-[60%] h-[60%] object-contain" />
          </div>
          <img 
            src={menu}
            alt="menu"
          ...
          />
                    <div className={`absolute top-[60px] right-0 left-0 bg-[#f3f3f8] z-10 shadow-secondary py-4 ${!toggleDrawer ? '-translate-y-[100vh]' : 'translate-y-0'} transition-all duration-700`}>
            <ul className="mb-4">
              {navlink.map((link) => (
                <li
                  key={link.name}
                  className={`flex p-4 ${isActive === link.name && 'bg-[#f3f3f8]'}`}
                  onClick={() => {
                ...
                >
                  ...
                  />
                  <p className={`ml-[20px] font-epilogue font-semibold text-[14px] ${isActive === link.name ? 'text-[#3B3456]' : 'text-[#808191]'}`}>
                    {link.name}
                  </p>
                </li>
              ))}
